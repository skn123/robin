...
class _TestInfo(object):
    ...
    def print_report(self, stream):
        ...
        if self._error != None:
            if not self._error[0].__name__.endswith('SkipTestException'):
...
class _XMLTestResult(unittest.TestResult):
    ...
    def print_report(self, stream, time_taken, out, err):
        ...
        """
        #count as errors only tests which failed not due to skipping them
        failed_counter=0
        for er in self.errors:
            if er[1].find('SkipTestException') == -1:
                failed_counter = failed_counter+1
        
        stream.write('<testsuite errors="%(e)d" failures="%(f)d" ' % \
            { "e": failed_counter, "f": len(self.failures) })
        ...
        #make sure the out and err strings do not contain illegal chars for XMLs
        out = out.replace('\x1b','');
        err = err.replace('\x1b','');
